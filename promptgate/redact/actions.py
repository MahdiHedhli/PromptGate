...
def apply_actions(text: str, findings: list[Finding], policy: Policy, vault: TokenVault, conversation_id: str | None = None) -> RedactionResult:
    ...
    blocked = [f.category for f in merged if not policy.is_audit_only(f.rule_id) and (policy.action_for(f.category) == "block" or not f.safe_replace)]
    action_log: list[dict] = []
    if blocked and policy.mode == "enforce":
        for finding in merged:
            action_log.append(_metadata(finding, policy.action_for(finding.category), policy, audit_only=policy.is_audit_only(finding.rule_id)))
    ...
    for finding in merged:
        ...
        audit_only = policy.is_audit_only(finding.rule_id)
        ...
        if audit_only:
            ...
            pieces.append(
                vault.token_for(
                    ...
                    mode=policy.tokenization.mode,
                    ttl_seconds=policy.tokenization.ttl_seconds,
                    max_conversations=policy.tokenization.max_conversations,
                )
            )
        else:
            pieces.append(text[finding.start:finding.end])
        cursor = finding.end
        action_log.append(_metadata(finding, action, policy, audit_only=audit_only))
...
def _metadata(finding: Finding, action: str, policy: Policy, audit_only: bool = False) -> dict:
    metadata = {
